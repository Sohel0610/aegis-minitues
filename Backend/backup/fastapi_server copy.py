...
# Add Pydantic models for Director Analytics
class DirectorAnalyticsResponse(BaseModel):
    din: Optional[str]
    name: str
    source_file: str
...
# Add endpoint to generate document summary
@app.post("/api/directors-disclosures/{disclosure_id}/generate-summary", response_model=SummaryGenerationResponse)
async def generate_disclosure_summary(disclosure_id: int):
    ...
    try:
        ...
        def generate_summary():
            ...
            # Try to import our LLM utilities
            try:
                import sys
...
# Add endpoint to get document summary
@app.get("/api/directors-disclosures/{disclosure_id}/summary", response_model=DocumentSummaryResponse)
...
# Director Analytics Endpoints
@app.get("/api/directors", response_model=List[DirectorAnalyticsResponse])
async def get_directors_analytics():
    """Get all directors for analytics"""
    try:
        from routes.director_data_analysis import get_all_directors
        ...
        return [DirectorAnalyticsResponse(**d) for d in directors]
    except Exception as e:
        logger.error(f"Error fetching directors analytics: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to fetch directors: {str(e)}")

@app.get("/api/company-count", response_model=CompanyCountResponse)
async def get_company_count_analytics():
    """Get company count statistics"""
    try:
        from routes.director_data_analysis import get_company_count
...
@app.get("/api/cross-directorship", response_model=List[CrossDirectorshipResponse])
async def get_cross_directorship_analytics():
    """Get cross-directorship information"""
    try:
        from routes.director_data_analysis import get_cross_directorship
        ...
        return [CrossDirectorshipResponse(**d) for d in cross_directorship]
    except Exception as e:
...
@app.get("/api/clustering", response_model=List[ClusteringResponse])
async def get_clustering_analytics():
    """Get director clustering information"""
    try:
        from routes.director_data_analysis import get_clustering
        ...
        return [ClusteringResponse(**c) for c in clustering]
    except Exception as e:
...
async def get_network_analytics():
    ...
    try:
        from routes.director_data_analysis import get_network
...
@app.get("/api/wtd-count", response_model=List[WTDCountResponse])
async def get_wtd_count_analytics():
    """Get whole-time director count"""
    try:
        from routes.director_data_analysis import get_wtd_count
        ...
        return [WTDCountResponse(**w) for w in wtd_count]
    except Exception as e:
...
@app.get("/api/companies-with-director-count", response_model=List[CompanyWithDirectorCountResponse])
...
# AI Assistant Models and Endpoints
import uuid
import json
import subprocess
import platform
...
@app.post("/ai-assistant/upload", response_model=TranscriptUploadResponse)
...
@app.post("/ai-assistant/generate-mom", response_model=MoMGenerationResponse)
        ...
        def process_transcript():
            ...
            prompt_data = {
                "messages": [
                    {
                        "role": "system",
                        "content": "You are an AI assistant that converts meeting transcripts into structured Meeting Minutes. Extract key information including title, date, attendees, agenda items, decisions made, action items with assignees, and next meeting details. Respond ONLY with valid JSON in the following format: {\"title\": \"Meeting Title\", \"date\": \"YYYY-MM-DD\", \"attendees\":[{\"name\": \"Attendee Name\", \"role\": \"Attendee Role\"}], \"agenda\":[\"Agenda Item 1\", \"Agenda Item 2\"], \"decisions\":[\"Decision 1\", \"Decision 2\"], \"action_items\":[{\"task\": \"Task Description\", \"assignee\": \"Assignee Name\"}], \"next_meeting\": \"Next meeting details\"}"
                    },
                    {
                        "role": "user",
            ...
            try:
                ...
                if use_groq:
                    ...
                    # Use shell script for Azure OpenAI
                    # Set environment variables for the subprocess
                    env = os.environ.copy()
                    env['USE_GROQ'] = 'false'
                    
                    result = subprocess.run([script_path, prompt_file], capture_output=True, text=True, cwd=os.path.dirname(script_path), env=env)
...
@app.get("/ai-assistant/status/{task_id}")
...
@app.get("/ai-assistant/mom/{task_id}")
async def get_mom_content(task_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get Meeting Minutes: {str(e)}")

@app.get("/ai-assistant/download/{task_id}")
async def download_mom_docx(task_id: str):
    ...
    try:
        ...
        logger.error(f"Error downloading MoM DOCX: {str(e)}")
