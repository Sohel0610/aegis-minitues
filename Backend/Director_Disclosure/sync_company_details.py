...
def init_company_schema():
    ...
    try:
        ...
        for col_name, col_type in [
            ...
            ("list_status", "TEXT"), ("last_sync", "TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP")
        ]:
    ...
    try:
        ...
        if isinstance(directors_list, list) and directors_list:
            ...
            for d in directors_list:
                ...
                # We no longer update external_associations from the Company API 
...
# ─── Single CIN Sync ─────────────────────────────────────────────────────
def sync_one(cin: str, idx: int = 1, total: int = 1) -> str:
    """
    Syncs a single CIN.
    ...
    try:
        ...
        if ok:
            ...
            print(f"{prefix} ✓  {cin:<22}  →  {display_name:<48} ({boards} dirs, {charges} charges)")
            return "ok"
        else:
            label = {
        ...
        print(f"{prefix} ✗  {cin:<22}  →  EXCEPTION: {str(e)[:50]}")
        log_failure(cin, ERROR_UNKNOWN, str(e))
        return ERROR_UNKNOWN


# ─── Batch Sync ──────────────────────────────────────────────────────────
def sync_all_cin_from_db():
    conn = get_db_connection()
    if not conn:
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT DISTINCT cin FROM directors_master.external_associations
    ...
    total = len(cins)
    print(f"\n  Found {total} unique CINs to sync.\n")
    print(f"  {'─'*70}")
    ...
    for idx, cin in enumerate(cins, 1):
        ...
        if result == "ok":
            ...
            stats["timeout"] += 1
        elif ERROR_API_STATUS in result:
            stats["api"] += 1
        ...
        # Stop if proxy is totally unreachable (all would fail)
        if stats["proxy"] >= 5:
            print(f"\n  [ABORT] Proxy unreachable for 5 consecutive CINs. Check VPN/proxy settings.")
            print(f"  Remaining CINs will be skipped. Run with --retry once network is restored.\n")
            break
...
def retry_failed():
    ...
    print(f"\n  Retrying {len(cins)} previously failed CINs...\n")
    ...
    _write_summary(stats, len(cins), time.time() - start, cins)
...
def _write_summary(stats: dict, total: int, elapsed: float, cins: list):
    ...
    lines = [
        ...
        f"  ✓  Successfully synced  : {stats['ok']}",
