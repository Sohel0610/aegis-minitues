...
class User(Base):
    ...
    meeting_metadata = relationship("MeetingMetadata", back_populates="user", cascade="all, delete-orphan")
...
class MeetingMetadata(Base):
    """Rich meeting identity extracted at upload time; powers long-term recall.

    Each uploaded document is optionally linked to exactly one MeetingMetadata
    row.  The fields stored here allow the retrieval pipeline to narrow results
    by date range, meeting type, participant, company, or topic — even months
    after the original upload.
    """
    __tablename__ = "chatbot_meeting_metadata"

    id = Column(Integer, primary_key=True, index=True)
    document_id = Column(Integer, ForeignKey("chatbot_documents.id"), unique=True, nullable=False)
    user_id = Column(Integer, ForeignKey("chatbot_users.id"), nullable=False, index=True)

    # Core meeting identity
    meeting_title = Column(String(500))
    meeting_date = Column(Date, index=True)
    meeting_type = Column(String(100), index=True)  # board_meeting, audit_committee, AGM, vendor_review, etc.
    company_name = Column(String(255), index=True)
    project_name = Column(String(255), index=True)

    # Participants
    participants = Column(JSON)          # [{"name": "...", "role": "..."}]
    chairperson = Column(String(255))

    # Content identity
    agenda_summary = Column(Text)        # Auto-extracted or user-provided
    key_topics = Column(JSON)            # ["vendor pricing", "compliance audit", ...]
    key_decisions = Column(JSON)         # ["Approved Q2 budget", ...]
    action_items_summary = Column(JSON)  # [{"task": "...", "owner": "...", "deadline": "..."}]

    # AI-generated meeting summary
    meeting_summary = Column(Text)       # 3-5 sentence summary of the entire document

    # Extraction tracking
    extraction_confidence = Column(String(20), default="auto")  # auto, user_verified, manual
    extracted_at = Column(DateTime, default=datetime.utcnow)
    user_edited = Column(DateTime)       # Set when user manually edits metadata

    document = relationship("Document", backref="meeting_metadata_rel", uselist=False)
    user = relationship("User", back_populates="meeting_metadata")
