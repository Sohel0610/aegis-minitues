        ...
        def increment():
            ...
            if conn:
                try:
                    ...
                    cursor.execute("UPDATE visit_tracking.visits SET count = count + 1, last_updated = CURRENT_TIMESTAMP WHERE id = 1")
                    conn.commit()
                finally:
                    conn.close()
    ...
    try:
        def fetch():
            ...
            if conn:
                try:
                    cursor = get_pg_cursor(conn)
                    cursor.execute("SELECT id, name, address, is_default, created_at FROM visit_tracking.places ORDER BY name")
