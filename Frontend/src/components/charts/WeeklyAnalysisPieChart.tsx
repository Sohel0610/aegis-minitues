...
interface WeeklyAnalysisPieChartProps {
  data: WeeklyData[];
}
...
const WeeklyAnalysisPieChart = ({ data }: WeeklyAnalysisPieChartProps) => {
  const [activeIndex, setActiveIndex] = useState<number | null>(null);

  // Aggregate data by entity_name for the pie chart
  const aggregatedData = data.reduce((acc, curr) => {
    const existing = acc.find(item => item.entity_name === curr.entity_name);
    if (existing) {
      existing.total_notifications += curr.total_notifications;
    } else {
      acc.push({
        week: curr.week,
    ...
    >
      <Card className="w-full" style={{
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <CardTitle className="text-lg font-semibold" style={{ color: '#010741' }}>
                Weekly Analysis Distribution
              </CardTitle>
              <CardDescription className="text-xs" style={{ color: 'rgba(1, 7, 65, 0.8)' }}>
                Distribution of notifications by entity
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-1">
          ...
          >
            <ChartContainer config={chartConfig} className="w-full h-64 sm:h-72 lg:h-80">
...
export default WeeklyAnalysisPieChart;